...
def add_players_received(players_file, players_to_add):
    players_file.write(players_to_add)

def get_filter_players_in_matches_function(matches_file, players_file, players_by_match, matches, output_queue):
    ...
    def filter_players_in_matches(queue, received_string, _, __):
        if received_string == MATCHES_SENTINEL:
            ...
            if identificator == JOIN_TO_REDUCERS_PLAYERS_IDENTIFICATOR:
                ...
                if not no_more_matches:
                    add_players_received(players_file, players_to_add)
...
def get_on_sentinel_callback_function(matches_file, send_sentinel_to_master_function):
...
def get_initial_matches_and_players(matches_file, players_file):
    matches = {}
    for match_id in matches_file.content:
        matches[match_id] = MATCH_PRESENT

    players_by_match = {}
    for player in players_file.content:
        match_id = player[FROM_CLIENT_PLAYER_MATCH_INDEX]
        # match not arrived
        if matches.get(match_id, None) is None:
            players_of_match = players_by_match.get(match_id, [])
            players_of_match.append(player)
            players_by_match[match_id] = players_of_match
...
def join_players_and_matches(input_queue, output_queue, send_sentinel_to_master_function):
    ...
    matches, players_by_match = get_initial_matches_and_players(
        matches_file, players_file)

    logger.info('Starting to receive players and matches in matches to join them.')
    input_queue.consume(
        get_filter_players_in_matches_function(
            matches_file, players_file, players_by_match, matches, output_queue
        ),
