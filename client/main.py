...
def get_print_matches_ids_function(matches_ids):
    ...
    def print_matches_ids(channel, method, properties, body):
        ...
        if received_string == SENTINEL_MESSAGE:
            print(f"Los IDs de matches que excedieron las dos horas de juego por pro players (average_rating > 2000) en los servers koreacentral, southeastasia y eastus son: {', '.join(matches_ids)}")
...
def request_long_matches(channel):
    ...
    channel.basic_consume(
        ...
        on_message_callback=get_print_matches_ids_function(matches_ids),
        auto_ack=True
    )
